#!/usr/bin/env python3
"""ascension_gate.py — the Stop-hook gate for an ascended deck-builder run.

**This file is dealt INTO target repos** at
``.claude/deck-builder/ascension_gate.py`` by ``/deck-builder:ascend``, and
wired as a Stop hook in the target repo's own ``.claude/settings.json``. Like
``record_play.py``, it is deliberately self-contained (no engine imports):
config it reads was written once at ascend-time, and it must keep enforcing
that tier even if the deck-builder plugin is later uninstalled.

Reads ``.claude/deck-builder-ascension.json`` (tier + lint/test commands,
written by ``ascend.py``) and enforces progressively more of the ladder:
...
CONFIG_FILENAME = "deck-builder-ascension.json"
...
def _config_path(repo: str) -> str:
    return os.path.join(repo, ".claude", CONFIG_FILENAME)
