---
description: The campfire — resolve a pending card reward (accept or skip), or when none is pending, review the deck for cards worth pruning. Use to upgrade or trim a spire run.
...
- Pending reward file: `${CLAUDE_PROJECT_DIR}/.spire/pending-reward.json`
- Play helper: `${CLAUDE_PROJECT_DIR}/.spire/bin/record_play.py`

## Step 1 — check for a pending reward

Read `${CLAUDE_PROJECT_DIR}/.spire/pending-reward.json`.
...
2. **For each accepted card**, deal it exactly as `/spire` step 7 does:
   ...
   `.spire/bin/record_play.py --name <name>`, and a short body you
   ...
   If `.spire/bin/record_play.py` doesn't exist yet in this repo,
...
5. Delete `.spire/pending-reward.json` — it's been resolved either way.
6. Show the result with `deck.py show --path "${CLAUDE_PROJECT_DIR}"`.

### If no pending reward exists

1. Read `${CLAUDE_PROJECT_DIR}/.spire/deck.json`.
2. Invoke the **deck-curator** agent for a general review, telling it in your
   prompt: the exact `deck.json` path (`${CLAUDE_PROJECT_DIR}/.spire/deck.json`),
